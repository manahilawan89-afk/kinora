...
import { FiMenu, FiSearch, FiUpload, FiX, FiSettings } from "react-icons/fi";
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-[#e5e5e5] bg-white pt-[env(safe-area-inset-top)] dark:border-white/10 dark:bg-[#0f0f0f]">
      <div className="flex h-14 items-center gap-2 px-3 sm:gap-3 sm:px-4">
        ...
        >
          <input
            className="h-10 w-full rounded-l-full border border-[#ccc] bg-white px-4 text-sm text-[#0f0f0f] outline-none placeholder:text-[#606060] focus:border-teal-700 dark:border-white/15 dark:bg-[#121212] dark:text-zinc-100"
            placeholder="Search"
            value={query}
        ...
        <div className="ml-auto flex items-center gap-1 sm:gap-2">
          ...
          <Link
            to="/settings"
            className="rounded-full p-2 text-[#0f0f0f] hover:bg-[#f2f2f2] dark:text-zinc-100 dark:hover:bg-white/10"
            title="Settings"
            aria-label="Settings"
          >
            <FiSettings size={18} />
          ...
          {user ? (
            <>
              ...
              <Link
                to="/settings"
                className="flex h-8 w-8 items-center justify-center overflow-hidden rounded-full bg-teal-700 text-xs font-medium text-white"
                title={user.username}
              >
                {user.avatar ? (
            ...
            <Link
              to="/login"
              className="rounded-full bg-teal-700 px-3.5 py-1.5 text-sm font-semibold text-white hover:bg-teal-800"
            >
              Sign in
            </Link>
          )}
      ...
      {mobileSearchOpen && (
        ...
        >
          <input
            autoFocus
            className="h-10 flex-1 rounded-full border border-[#ccc] bg-white px-4 text-sm outline-none focus:border-teal-700 dark:border-white/15 dark:bg-[#121212]"
            placeholder="Search"
            value={query}
            onChange={(e) => setQuery(e.target.value)}
          />
